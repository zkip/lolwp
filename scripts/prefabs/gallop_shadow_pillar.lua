local assets = {Asset("ANIM", "anim/shadow_pillar.zip")}
...
local prefabs = {
  "sanity_raise", "sanity_lower", "shadow_pillar_base_fx", "ocean_splash_med1",
  "ocean_splash_med2"
}

local prefabs_spell = {
  "shadow_pillar", "shadow_pillar_target", "shadow_glob_fx"
}
...
local function _CalcTargetDuration(target)
  return target ~= nil and
           ((target:HasTag("epic") and TUNING.SHADOW_PILLAR_DURATION_BOSS) or
             (target:HasTag("player") and TUNING.SHADOW_PILLAR_DURATION_PLAYER)) or
           TUNING.SHADOW_PILLAR_DURATION
end
-- changed only for this
local function CalcTargetDuration(target) return _CalcTargetDuration(target) / 2 end
...
local function GetNextFlipped()
  if math.random() < .65 then LAST_FLIPPED = not LAST_FLIPPED end
  return LAST_FLIPPED
end

local function DoSplash(inst)
  if TheWorld.Map:IsOceanAtPoint(inst.Transform:GetWorldPosition()) then
    SpawnPrefab("ocean_splash_med" .. tostring(math.random(2))).Transform:SetPosition(
...
local function DoLower(inst)
  ...
  if inst.base ~= nil then inst.base:DoTaskInTime(2 * FRAMES, inst.base.KillFX) end
  ...
  if data ~= nil then
    if data.name == "delay" then
      ...
      for _, v in ipairs(overlaps) do
        if v ~= inst and v.persists then Pillar_OnDispell(v) end
      end

      local t = CalcTargetDuration(inst.components.entitytracker:GetEntity(
                                     "target"))
      inst.components.timer:StartTimer("lifetime", t)
      inst.components.timer:StartTimer("warningtime",
                                       math.max(0, t - WARNING_TIME))

    elseif data.name == "warningtime" then
      inst.AnimState:PlayAnimation("shake" .. tostring(inst.variation), true)
      inst.SoundEmitter:PlaySound("maxwell_rework/shadow_pillar/rumble",
                                  "rumble")
    ...
    elseif data.name == "lifetime" then
      if inst._delayraisetask ~= nil then
        ...
        SpawnPrefab("sanity_lower").Transform:SetPosition(
          inst.Transform:GetWorldPosition())
        DoLower(inst)
      end
    end
...
local function Pillar_SetTarget(inst, target, hasplatform)
  ...
  if oldtarget ~= nil then
    if inst._ondispell ~= nil then
      inst:RemoveEventCallback("dispell_shadow_pillars", inst._ondispell,
                               oldtarget)
      inst._ondispell = nil
    end
    ...
    if inst._ontargetremoved ~= nil then
      ...
      inst:RemoveEventCallback("remove_shadow_pillars", inst._ontargetremoved,
                               oldtarget)
      inst._ontargetremoved = nil
    end
    if inst._reducetime ~= nil then
      inst:RemoveEventCallback("reduce_shadow_pillars_time", inst._reducetime,
                               target)
...
local function Pillar_OnLoad(inst, data)
  if inst.components.timer:TimerExists("lifetime") then
    ...
    if not inst.components.timer:TimerExists("warningtime") then
      ...
      if target ~= nil and
        not (target.components.health ~= nil and
...
local function Pillar_OnLoadPostPass(inst, ents, data)
  local target = inst.components.entitytracker:GetEntity("target")
  if target ~= nil then Pillar_OnSetTarget(inst, target) end
...
local function DoRipple(inst, ripples, map, x, z)
  if inst.AnimState:IsCurrentAnimation("idle") and map:GetPlatformAtPoint(x, z) ==
    nil then
    ripples.AnimState:PlayAnimation(math.random() < .5 and "no_splash" or
                                      "no_splash2")
  end
end
...
local function base_fn()
  ...
  if not TheNet:IsDedicated() then inst:DoTaskInTime(1, TryRipples) end
...
local function Target_Update(inst, x, z, target, attackers)
  ...
  if count > 0 then
    ...
    dt = Lerp(TUNING.SHADOW_PILLAR_BREAK_MULT.MIN,
              TUNING.SHADOW_PILLAR_BREAK_MULT.MAX, dt)
...
local function Target_OnSetTarget(inst, target)
  ...
  local function ontargetattacked(target, data)
    ...
    if attacker ~= nil then
      ...
      if not attacker:HasTag("player") then attacker = nil end
    end
...
local function Target_SetTarget(inst, target, radius, hasplatform)
  if target ~= nil then
    ...
    if hasplatform then inst:RemoveTag("ignorewalkableplatforms") end
...
local function Target_OnLoadPostPass(inst, ents, data)
  local target = inst.components.entitytracker:GetEntity("target")
  if target ~= nil and (inst.components.timer:TimerExists("lifetime") or
    inst.components.timer:TimerExists("delay")) then
    Target_OnSetTarget(inst, target)
  else
    -- bad save data
    inst.persists = false
...
local function TryFX(inst, offsets, map)
  ...
  while true do -- should we limit number of tries?
    ...
    if map:IsPassableAtPoint(x, 0, z, true) and
      not map:IsGroundTargetBlocked(Vector3(x, 0, z)) then
...
local function StartFX(inst)
  ...
  for i = 1, 3 do
    ...
    for j = 1, count do
      angle = angle + delta
      table.insert(offsets, Vector3(math.cos(angle) * radius, 0,
...
local function IsNearOther(pt, newpillars)
  for i, v in ipairs(newpillars) do
    if distsq(pt.x, pt.z, v.x, v.z) < 1 then return true end
  end
  return false
end
...
local function DoPillarsTarget(target, caster, item, newpillars, map, x0, z0)
  ...
  local padding = (target:HasTag("epic") and 1) or
                    (target:HasTag("smallcreature") and 0) or .75
  ...
  local ent = SpawnPrefab("gallop_shadow_pillar_target")
...
local SPELL_NO_TAGS_PVP = {
  "INLIMBO", "notarget", "flight", "invisible", "notraptrigger", "projectile"
}
...
local function DoPillars(inst, targets, newpillars)
  local map = TheWorld.Map
  local caster = inst.caster ~= nil and inst.caster:IsValid() and inst.caster or
                   nil
  ...
  local ents = TheSim:FindEntities(x, 0, z, AOE_RADIUS, SPELL_MUST_TAGS,
                                   TheNet:GetPVPEnabled() and SPELL_NO_TAGS_PVP or
                                     SPELL_NO_TAGS)
...
local function spell_fn()
  ...
  inst.SoundEmitter:PlaySound("maxwell_rework/shadow_magic/shadow_goop_ground",
                              "loop")
  StartFX(inst)
...
return Prefab("gallop_shadow_pillar", pillar_fn, assets, prefabs),
       Prefab("gallop_shadow_pillar_target", target_fn),
       Prefab("gallop_shadow_pillar_spell", spell_fn, nil, prefabs_spell)
