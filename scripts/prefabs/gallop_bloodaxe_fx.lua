...
local function InitEnvelope()
  ...
  EnvelopeManager:AddColourEnvelope(COLOUR_ENVELOPE_NAME, {
    {0, IntColour(10, 10, 10, 100)}, {.1, IntColour(10, 10, 10, 200)},
    {.4, IntColour(10, 10, 10, 180)}, {.5, IntColour(10, 10, 10, 200)},
    {.9, IntColour(10, 10, 10, 100)}, {1, IntColour(10, 10, 10, 0)}
  })
...
local function emit_sparkle_fn(effect, sphere_emitter)
  local vx, vy, vz = .005 * UnitRand(), 0.05 + .004 * UnitRand(),
                     .005 * UnitRand()
  ...
  local ang_vel = is_gear and
                    ((math.random() < 0.5 and 11 or -11) + UnitRand() * 2) or
                    (UnitRand() * 5)
  local angle = math.random() < 0.5 and 180 or 0
  local v_offset = 0

  effect:AddRotatingParticleUV(0, lifetime, -- lifetime
  px, py, pz, -- position
  vx, vy, vz, -- velocity
...
local function fn()
  ...
  EmitterManager:AddEmitter(inst, nil, function()
    ...
    if not (owner:HasTag("attack") or
      owner.AnimState:IsCurrentAnimation("pocketwatch_atk_pre") or
      owner.AnimState:IsCurrentAnimation("pocketwatch_atk_pre_2") or
      owner.AnimState:IsCurrentAnimation("pocketwatch_atk")) then
      local dist_moved = inst:GetPosition() - inst.last_pos
      local move = dist_moved:Length()
      move = math.clamp(move * 6, 0, 1)

      local low_pps = IsOld(owner) and 3 or IsYoung(owner) and 1 or 2

      local high_pps = IsOld(owner) and 6 or IsYoung(owner) and 1 or 3
...
return Prefab("gallop_bloodaxe_fx", fn, assets)
